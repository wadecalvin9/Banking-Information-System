...
const typeStyle = { Savings: "bg-blue-50 text-blue-600", Current: "bg-purple-50 text-purple-600", "Fixed Deposit": "bg-amber-50 text-amber-600" };
            ...
            <tbody className="divide-y divide-slate-50">
              {accounts.map((acc) => (
                <tr key={acc.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-3.5"><span className={`px-2.5 py-1 rounded-full text-xs font-medium ${typeStyle[acc.type]}`}>{acc.type}</span></td>
                  ...
                  <td className="px-6 py-3.5"><span className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusStyle[acc.status]}`}>{acc.status}</span></td>

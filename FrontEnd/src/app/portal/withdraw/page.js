...
export default function WithdrawPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (step === 2) {
      setLoading(true);
      try {
        // Backend uses 'to_account' in withdrawal to mean the account being withdrawn from
        const res = await authFetch("http://localhost:8080/api/withdraw", {
          method: "POST",
          body: JSON.stringify({
        ...
        if (res.ok) {
          ...
          alert(errorData || "Withdrawal failed. Check your balance and PIN.");
        }
      } catch (err) {
        console.error(err);
  ...
  if (step === 3) {
    return (
      <div className="max-w-md mx-auto text-center py-16">
        ...
        <h2 className="text-2xl font-bold text-slate-800">Withdrawal Successful!</h2>
        <p className="text-slate-500 mt-2 text-sm">Your withdrawal of <span className="font-semibold text-slate-700">KES {Number(form.amount).toLocaleString()}</span> has been processed.</p>
        <p className="text-xs text-slate-400 mt-1">Reference: WTH-00{txnId || 'PENDING'}</p>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Take money out of your NexaBank account</p>
      </div>

      {/* Steps */}
      <div className="flex items-center gap-2">
        {["Withdrawal Details", "Confirm"].map((label, i) => (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        {step === 1 && (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Source Account</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Amount to Withdraw (KES)</label>
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit} className="space-y-5">
            <h3 className="font-semibold text-slate-800">Confirm Withdrawal</h3>
            <div className="bg-slate-50 rounded-xl p-4 space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-slate-500">From Account</span><span className="font-medium text-slate-700">ACC-00{form.toAccount}</span></div>
